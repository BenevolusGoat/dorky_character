...
function VeeHelper.SmoothLerp(first, second, percent, smoothIn, smoothOut)
	if smoothIn then
...
function VeeHelper.GetClosestDoorSlotPos(pos)
	...
	for doorSlot = 0, DoorSlot.NUM_DOOR_SLOTS do
		...
		if door ~= nil and door:IsOpen() then
			local doorPos = game:GetRoom():GetDoorSlotPosition(doorSlot)
...
function VeeHelper.IsInStartingRoom()
	...
	if roomIndex == startingRoomIndex
		...
		if game:GetFrameCount() == 0 or
			(game:GetFrameCount() > 0 and level:GetCurrentRoom():IsFirstVisit() == true) then
			return true
		end
	end
	return false
...
---@param dir integer
function VeeHelper.DirectionToVector(dir)
